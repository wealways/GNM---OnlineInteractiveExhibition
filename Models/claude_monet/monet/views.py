...
def transfer(content_img: str, sessionkey: str):
    ai_dir = os.path.join(settings.BASE_DIR, 'monet', 'style_transfer')
    print(f'모델 저장 위치: {ai_dir}')

    # style image 위치
    style_img = os.path.join(settings.BASE_DIR, 'static', 'images', 'the-la-rue-bavolle-at-honfleur.jpg')
    # model dir 위치
    models = os.path.join(settings.BASE_DIR, 'monet', 'style_transfer')
    # vgg19
    vgg = os.path.join(models, 'vgg_normalised.pth')
    # decoder - monet
    decoder = os.path.join(models, 'claude_monet_160000.tar')
    # 저장할 위치(output)
    ...
    # 산출물 파일명
    output_image_file_name = f'monet_{content_img_path.stem}_output.jpg'
